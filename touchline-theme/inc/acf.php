...
if (!function_exists('touchline_get_testimonial_data')) {
  /**
   * Get testimonial data for rendering.
   *
   * @param int $post_id Post ID.
   * @return array
   */
  function touchline_get_testimonial_data($post_id)
  {
    $data = array(
      'quote' => '',
      'name' => '',
    ...
    if (function_exists('get_field')) {
      $data['quote'] = (string) get_field('quote', $post_id);
      $data['name'] = (string) get_field('name', $post_id);
      $data['role'] = (string) get_field('role', $post_id);
      $data['company'] = (string) get_field('company', $post_id);
      ...
      if (is_array($headshot) && !empty($headshot['url'])) {
        ...
        $data['headshot_url'] = wp_get_attachment_image_url((int) $headshot, 'thumbnail');
      }
    } else {
      $post = get_post($post_id);
      if ($post) {
        $data['quote'] = $post->post_content;
        $data['name'] = $post->post_title;
      }

      $thumb_id = get_post_thumbnail_id($post_id);
      if ($thumb_id) {
        $data['headshot_url'] = wp_get_attachment_image_url($thumb_id, 'thumbnail');
      }
    }
